/**
 * @file netio_https_server_demo.cpp
 * @brief 使用NetIO创建HTTPS服务器的demo
 * 
 * 这个文件演示了如何使用NetIO库创建一个HTTPS服务器。
 * 服务器监听指定端口，处理JSON-RPC请求，使用SSL/TLS加密。
 * 
 * 使用方法:
 *   ./netio_https_server_demo [cert_file] [key_file] [host] [port] [endpoint] [ca_file]
 * 
 * 默认值:
...
void signalHandler(int signal) {
    ...
    auto logger = mcp_logger::getModuleLogger("netio_https_server");
...
public:
    bool handleRequest(const Message& request, Message& response) override {
        ...
        response.headers["X-Server"] = "NetIO-HTTPS-Server/1.0";
...
int main(int argc, char* argv[]) {
    ...
    log_config.file_path = "logs/netio_https_server_demo.log";
    ...
    auto logger = mcp_logger::getModuleLogger("netio_https_server");
    logger->info("🚀 Starting NetIO HTTPS Server Example");
    ...
    logger->info("  cert_file: " + cert_file);
    ...
    if (!fileExists(cert_file)) {
        logger->error("❌ Certificate file not found: " + cert_file);
        logger->error("💡 To generate a self-signed certificate for testing:");
        logger->error("   openssl req -x509 -newkey rsa:4096 -keyout " + key_file + " -out " + cert_file + " -days 365 -nodes");
        return 1;
    }
    
    if (!fileExists(key_file)) {
        logger->error("❌ Key file not found: " + key_file);
        logger->error("💡 To generate a self-signed certificate for testing:");
        logger->error("   openssl req -x509 -newkey rsa:4096 -keyout " + key_file + " -out " + cert_file + " -days 365 -nodes");
        return 1;
    }
    ...
    if (!ca_file.empty() && !fileExists(ca_file)) {
        ...
        logger->error("💡 CA file is optional, you can omit it");
        return 1;
    }
    ...
    try {
        ...
        if (!g_netio->startServer(config)) {
            ...
            logger->error("  1. Certificate and key files are valid and accessible");
            logger->error("  2. Port " + std::to_string(port) + " is not in use");
            logger->error("  3. OpenSSL support is enabled during compilation");
            g_netio.reset();
            return 1;
        }
        ...
        int wait_count = 0;
        const int max_wait = 100; // 最多等待10秒
        ...
        if (!server_started) {
            ...
            logger->error("  3. Invalid certificate or key file");
            logger->error("  4. Network configuration issue");
            logger->error("  5. OpenSSL support is not enabled");
        ...
        logger->info("✅ NetIO HTTPS Server started successfully");
        ...
        logger->info("💡 Note: For self-signed certificates, clients may need to disable certificate verification");
