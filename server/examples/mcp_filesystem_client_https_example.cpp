/**
 * @file mcp_filesystem_client_https_example.cpp
 * @brief MCP FileSystem Client HTTPS 传输示例
 * 
 * 这个示例展示了如何使用MCPClient类通过HTTPS传输方式连接到文件系统服务器。
...
void signalHandler(int signal) {
    if (signal == SIGINT || signal == SIGTERM) {
        auto logger = getModuleLogger("mcp_filesystem_client_https");
...
void printClientInfo() {
    auto logger = getModuleLogger("mcp_filesystem_client_https");
    logger->info("🚀 MCP FileSystem Client HTTPS 示例");
    logger->info("=================================");
    logger->info("📋 功能特性:");
    logger->info("  ✅ 基于HTTPS传输的文件系统客户端");
...
void printUsage() {
    auto logger = getModuleLogger("mcp_filesystem_client_https");
    logger->info("📖 使用说明:");
    logger->info("  1. 客户端通过HTTPS连接到服务器进行加密通信");
    ...
    logger->info("  4. HTTPS端点: https://<host>:<port><endpoint>");
...
void initializeLogger() {
    ...
    config.file_path = "logs/mcp_filesystem_client_https.log";
...
void demonstrateClientFeatures(MCPClient& client) {
    auto logger = getModuleLogger("mcp_filesystem_client_https");
    logger->info("开始演示HTTPS客户端功能");
    
    logger->info("🔧 HTTPS客户端功能演示:");
    ...
    // 3.5 创建新文件
    std::string test_content = "这是通过HTTPS客户端创建的文件\n";
    test_content += "创建时间: " + std::to_string(std::time(nullptr)) + "\n";
    test_content += "传输方式: HTTPS (安全超文本传输协议)\n";
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--host" && i + 1 < argc) {
            ...
            logger->info("  --port <port>           设置服务器端口 (默认: 8443)");
            ...
            logger->info("");
            logger->info("注意: 对于自签名证书，建议使用 --no-verify-peer 和 --no-verify-hostname");
    ...
    if (!ca_file.empty()) {
        logger->info("🔐 CA证书: " + ca_file);
    }
    if (!verify_peer) {
        logger->warning("⚠️  对等方证书验证已禁用 (仅用于测试)");
    }
    if (!verify_hostname) {
        logger->warning("⚠️  主机名验证已禁用 (仅用于测试)");
    }
    
    logger->info("🚀 启动 MCP FileSystem Client HTTPS 示例");
    ...
    try {
        ...
        https_config.mode = mcp_transport::TransportMode::CLIENT;
        https_config.name = "mcp_filesystem_client_https";
        
        // 日志配置
        https_config.enable_logging = true;
        https_config.log_level = 0;  // 0=DEBUG, 1=INFO, 2=WARNING, 3=ERROR
        https_config.log_file_path = "logs/mcp_filesystem_client_https_transport.log";
        ...
        https_config.sse.auto_reconnect = true;
        https_config.sse.reconnect_interval = 5;
        https_config.sse.user_agent = "MCP-FileSystem-Client-HTTPS/1.0";
        ...
        https_config.sse.headers["User-Agent"] = "MCP-FileSystem-Client-HTTPS/1.0";
        ...
        logger->info("  - 自动重连: " + std::string(https_config.sse.auto_reconnect ? "启用" : "禁用"));
        if (!ca_file.empty()) {
            logger->info("  - CA证书: " + ca_file);
        }
        logger->info("  - 验证对等方: " + std::string(verify_peer ? "启用" : "禁用"));
        logger->info("  - 验证主机名: " + std::string(verify_hostname ? "启用" : "禁用"));
        
        // HTTPS配置已通过TransportConfig.https传递，MCPClient会自动设置
        // 设置transport类型
        client->setTransportType(mcp_transport::TransportType::HTTPS);
        
        // 启动连接
        logger->info("🚀 连接到服务器...");
        logger->info("🚀 正在连接到服务器...");
        
        if (!client->connectWithConfig(https_config)) {
            logger->error("❌ 连接失败");
            logger->error("❌ 连接失败，请检查:");
            logger->error("  1. 服务器是否正在运行");
            logger->error("  2. 服务器地址是否正确: https://" + host + ":" + std::to_string(port) + endpoint);
            logger->error("  3. 网络连接是否正常");
            logger->error("  4. SSL证书配置是否正确");
            logger->error("  5. 对于自签名证书，尝试使用 --no-verify-peer 和 --no-verify-hostname");
            return 1;
        }
        
        // 注意：由于MCPClient的封装，我们无法在连接后直接设置HTTPS配置
        // 如果需要设置HTTPS配置（如CA证书、验证选项等），可能需要：
        // 1. 修改MCPClient来支持HTTPS配置参数
        // 2. 扩展TransportConfig来包含HTTPS配置
        // 3. 或者通过其他方式传递HTTPS配置
        ...
        // 设置客户端信息
        client->setClientInfo("mcp-filesystem-client-https", "1.0.0");
        
        // 创建初始化参数
        InitializeParams init_params;
        init_params.protocolVersion = "2025-11-25";
        init_params.clientInfo.name = "mcp-filesystem-client-https";
