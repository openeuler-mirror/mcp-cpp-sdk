/**
 * @file http_transport_gtest.cpp
 * @brief HttpTransport Google Test 测试文件
 * @author MCP SDK Team
 * @date 2024
 */
...
protected:
    void SetUp() override {
        ...
        // 设置默认配置
        config_.type = TransportType::HTTP;
        config_.mode = TransportMode::CLIENT;
        config_.sse.host = "localhost";
        config_.sse.port = 8080;
        config_.sse.protocol = netio::Protocol::HTTP;
 ...
 * @brief 测试基本构造和析构
 */
TEST_F(HttpTransportTest, BasicConstruction) {
    EXPECT_FALSE(transport_->isRunning());
    EXPECT_EQ(transport_->getStatus(), TransportStatus::DISCONNECTED);
    EXPECT_EQ(transport_->getTransportTypeName(), "http");
}

/**
 * @brief 测试获取传输类型名称
 */
TEST_F(HttpTransportTest, GetTransportTypeName) {
    EXPECT_EQ(transport_->getTransportTypeName(), "http");
}
 ...
 */
TEST_F(HttpTransportTest, GetConnectionInfoNotRunning) {
    ConnectionInfo info = transport_->getConnectionInfo();
    EXPECT_EQ(info.status, TransportStatus::DISCONNECTED);
    EXPECT_EQ(info.type, TransportType::HTTP);
}

/**
 * @brief 测试设置消息回调
 */
TEST_F(HttpTransportTest, SetMessageCallback) {
 ...
 * @brief 测试设置服务端请求处理器
 */
TEST_F(HttpTransportTest, SetServerRequestHandler) {
 ...
 */
TEST_F(HttpTransportTest, SetSseServerRequestHandler) {
 ...
 * @brief 测试设置连接请求处理器
 */
TEST_F(HttpTransportTest, SetConnectionRequestHandler) {
 ...
 * @brief 测试重复启动
 */
TEST_F(HttpTransportTest, DoubleStart) {
    // 第一次启动应该失败（因为没有实际服务器）
 ...
 */
TEST_F(HttpTransportTest, StopNotRunning) {
 ...
 * @brief 测试发送请求（未启动状态）
 */
TEST_F(HttpTransportTest, SendRequestNotRunning) {
 ...
 */
TEST_F(HttpTransportTest, SendJsonRpcMessageNotRunning) {
 ...
 * @brief 测试健康检查（未启动状态）
 */
TEST_F(HttpTransportTest, HealthCheckNotRunning) {
 ...
 * @brief 测试获取扩展统计信息
 */
TEST_F(HttpTransportTest, GetExtendedStatistics) {
 ...
 * @brief 测试获取统计信息
 */
TEST_F(HttpTransportTest, GetStatistics) {
 ...
 * @brief 测试配置更新
 */
TEST_F(HttpTransportTest, UpdateConfig) {
    TransportConfig new_config = config_;
    new_config.sse.port = 9090;
    transport_->updateConfig(new_config);
    
    TransportConfig current_config = transport_->getConfig();
    EXPECT_EQ(current_config.sse.port, 9090);
}

/**
 * @brief 测试获取配置
 */
TEST_F(HttpTransportTest, GetConfig) {
    ...
    EXPECT_EQ(config.type, TransportType::HTTP);  // 现在应该是HTTP
    EXPECT_EQ(config.mode, TransportMode::CLIENT);
}
 ...
 * @brief 测试消息队列操作
 */
TEST_F(HttpTransportTest, MessageQueueOperations) {
 ...
 * @brief 测试连接回调
 */
TEST_F(HttpTransportTest, ConnectionCallback) {
    bool callback_called = false;
 ...
 * @brief 测试错误回调
 */
TEST_F(HttpTransportTest, ErrorCallback) {
    bool callback_called = false;
 ...
 */
TEST_F(HttpTransportTest, SendRequestAsyncNotRunning) {
    TransportMessage msg;
    msg.content = "test";
    msg.method = "POST";
    msg.path = "/";
 ...
 */
TEST_F(HttpTransportTest, TransportMessageConversion) {
    TransportMessage msg;
    msg.id = "test_id";
 ...
 * @brief 测试服务端模式配置
 */
TEST_F(HttpTransportTest, ServerModeConfig) {
    ...
    EXPECT_EQ(current_config.sse.port, 8080);
