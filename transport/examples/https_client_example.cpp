...
TransportMessage sendRequestWithSession(TransportBase& transport, ClientState& state,
    ...
    auto logger = mcp_logger::getModuleLogger("https_client");
...
void testEchoRequest(TransportBase& transport, ClientState& state, const std::string& endpoint) {
    ...
    request["jsonrpc"] = "2.0";
    request["method"] = "echo";
    request["params"] = "Hello from HTTPS client!";
...
void testNotification(TransportBase& transport, ClientState& state) {
    ...
    nlohmann::json params;
    params["message"] = "This is a test log message from HTTPS client";
...
int main(int argc, char* argv[]) {
    ...
    log_config.file_path = "logs/https_client_example.log";
    ...
    auto logger = mcp_logger::getModuleLogger("https_client");
    logger->info("🚀 Starting HttpsTransport Client Example");
    ...
    if (argc > 5) {
        verify_peer = (std::string(argv[5]) == "true" || std::string(argv[5]) == "1");
    }
    if (argc > 6) {
        verify_hostname = (std::string(argv[6]) == "true" || std::string(argv[6]) == "1");
    }
    
    logger->info("📡 Connecting to server: https://" + host + ":" + std::to_string(port) + endpoint);
    if (!ca_file.empty()) {
        logger->info("🔐 Using CA certificate: " + ca_file);
    }
    if (!verify_peer) {
        logger->warning("⚠️  Peer certificate verification is disabled (for testing with self-signed certificates)");
    }
    if (!verify_hostname) {
        logger->warning("⚠️  Hostname verification is disabled (for testing with self-signed certificates)");
    }
    ...
    // 创建传输实例（使用工厂类）
    std::unique_ptr<HttpsTransport> transport = TransportFactory::createHttpsTransport();
    ...
    transport->setHttpsConfig(https_config);
    ...
    if (!transport->start(config)) {
        logger->error("❌ Failed to start HttpsTransport client");
        logger->error("💡 Make sure the server is running on https://" + host + ":" + std::to_string(port));
        logger->error("💡 For self-signed certificates, you may need to:");
        logger->error("   - Provide CA certificate file with --ca-file option");
        logger->error("   - Or disable verification (not recommended for production)");
        return 1;
    }
    
    logger->info("✅ HttpsTransport Client started successfully");
