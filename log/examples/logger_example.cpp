...
void testBasicLogging() {
    ...
    // 获取模块日志记录器
    auto module_logger_ = logger.getModuleLogger("logger_example");
    ...
    // 测试各种日志级别
    module_logger_->debug("这是一条调试信息");
    module_logger_->info("这是一条信息日志");
    module_logger_->warning("这是一条警告信息");
    module_logger_->error("这是一条错误信息");
    module_logger_->fatal("这是一条致命错误信息");
...
void testModuleLogging() {
    ...
    // 为模块化测试设置专门的日志文件
    logger.setLogFile("logs/module_test.log");
...
void testThreadSafety() {
    ...
    // 为线程安全测试设置专门的日志文件
    logger.setLogFile("logs/thread_test.log");
    ...
    for (int i = 0; i < 5; ++i) {
        threads.emplace_back([i, &module_logger_]() {
            for (int j = 0; j < 10; ++j) {
                module_logger_->info("线程 " + std::to_string(i) + " 第 " + std::to_string(j) + " 条消息");
...
void testLogRotation() {
    ...
    config.max_files = 3;
    config.file_path = "logs/rotation_test.log";
    logger.initialize(config);
    
    // 写入大量日志以触发轮转
    for (int i = 0; i < 100; ++i) {
        module_logger_->info("这是一条用于测试日志轮转的消息，编号: " + std::to_string(i));
    }
...
void testDifferentFormats() {
    ...
    // 为格式测试设置专门的日志文件
    logger.setLogFile("logs/format_test.log");
    ...
    module_logger_->info("详细格式日志");
    
    // 测试自定义格式
    logger.setFormat("*** {level} *** {message} ***");
    module_logger_->warning("自定义格式日志");
...
void testLogLevels() {
    ...
    // 为日志级别测试设置专门的日志文件
    logger.setLogFile("logs/level_test.log");
...
int main() {
    ...
    try {
        ...
        std::cout << "请检查 logs/ 目录下的日志文件查看输出结果:" << std::endl;
        std::cout << "  - logs/logger_example.log (基本日志测试)" << std::endl;
        std::cout << "  - logs/module_test.log (模块化日志测试)" << std::endl;
        std::cout << "  - logs/thread_test.log (线程安全测试)" << std::endl;
        std::cout << "  - logs/rotation_test.log (日志轮转测试)" << std::endl;
        std::cout << "  - logs/format_test.log (格式测试)" << std::endl;
        std::cout << "  - logs/level_test.log (日志级别测试)" << std::endl;
