/**
 * @file netio_mcp_demo.cpp
 * @brief 使用netio接口实现MCP请求的demo
 * 
 * 这个文件演示了如何使用netio库发送HTTP/HTTPS请求到MCP服务器。
 * 支持HTTP和HTTPS两种协议。
 */
...
// 创建MCP工具列表请求
Message create_mcp_tools_list_request() {
    // 创建JSON请求数据
    json request_data = {
        {"jsonrpc", "2.0"},
        {"method", "tools/list"},
...
int main() {
    ...
    try {
        ...
        // 检查连接状态
        auto conn_info = netio_client.get_connection_info();
        ...
        // 创建MCP请求
        Message request = create_mcp_tools_list_request();
        ...
        Message response = netio_client.send_request_sync(request);
        ...
        // 显示统计信息
        std::cout << "\n📊 统计信息:" << std::endl;
        auto stats = netio_client.get_statistics();
